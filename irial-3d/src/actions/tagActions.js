import { FETCH_TAGS, UPDATE_MODEL_TAGS, UPDATE_PROJECT_TAGS, UPDATE_TEXTURE_TAGS, UPDATE_SCENE_TAGS } from "./types";
  ...
  export const toggleProjectTags = (project, tag, toogle ) => async dispatch => {
    const lang = getLanguage();
      ...
      if(toogle && project.tags.length < 4) {
        const tagsDb =  await DashBoard.get("/tags/add_project_tag.php"+ generatePHPParameters({lang, idProject: project.id, idTag: tag}));
        var tags = tagsDb.data.slice();
      }
      
      if(!toogle && project.tags.length > 0)  {
        const tagsDb =  await DashBoard.get("/tags/delete_project_tag.php"+ generatePHPParameters({lang, idProject: project.id, idTag: tag}));
        var tags = tagsDb.data.slice();
      }
  ...
  export const toggleTextureTags = (texture, tag, toogle ) => async dispatch => {
    const lang = getLanguage();
      ...
      if(toogle && texture.tags.length < 4) {
        const tagsDb =  await DashBoard.get("/tags/add_texture_tag.php"+ generatePHPParameters({lang, idTexture: texture.id, idTag: tag}));
        var tags = tagsDb.data.slice();
      }
      
      if(!toogle && texture.tags.length > 0)  {
        const tagsDb =  await DashBoard.get("/tags/delete_texture_tag.php"+ generatePHPParameters({lang, idTexture: texture.id, idTag: tag}));
        var tags = tagsDb.data.slice();
      }
  ...
  export const toggleSceneTags = (scene, tag, toogle ) => async dispatch => {
    const lang = getLanguage();
      ...
      if(toogle && scene.tags.length < 4) {
        const tagsDb =  await DashBoard.get("/tags/add_scene_tag.php"+ generatePHPParameters({lang, idScene: scene.id, idTag: tag}));
        var tags = tagsDb.data.slice();
      }
      
      if(!toogle && scene.tags.length > 0)  {
        const tagsDb =  await DashBoard.get("/tags/delete_scene_tag.php"+ generatePHPParameters({lang, idScene: scene.id, idTag: tag}));
        var tags = tagsDb.data.slice();
      }
