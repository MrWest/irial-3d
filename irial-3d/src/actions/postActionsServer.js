import { FETCH_POSTS, SELECT_POST, FETCH_BLOG_CATEGORIES, SET_LANG_ES, SET_LANG_EN } from "./types";
...
export const fetchPostsServer =  async reduxStore => {
  ...
  const postsFullData = await WordpressRestAPI.get(`/posts/?filter%5Bmeta_key%5D=type&filter%5Bmeta_value%5D=VinalesTraveler&filter%5Bmeta_key%5D=language&filter%5Bmeta_value%5D=${language}`).then(res => res.data);
  const posts = postsFullData.map(post => {
    const { id, slug, title, excerpt, acf: { headliner, category, readtime, image, description } } = post;
    return {
      id,
      slug,
  ...
  export const fetchPosts =  () => async dispatch => {
    ...
    const posts = postsFullData.map(post => {
      const { id, slug, title, excerpt, _embedded, acf: { headliner, category, readtime, image, description } } = post;
      return {
        id,
        slug,
        title: title.rendered,
...
const GetSmoothPost = (post, comments = [], relatedPosts = []) => {
  ...
  const postRslt = {
    ...
    readtime,
    image: image.url,
    author: { nickname: author_nickname, fullname: author_fullname, avatar: author_avatar, title: author_title }
  };
...
const slugWorkoutLang = (slug) => {
  ...
  const postsFullData = await WordpressRestAPI.get(`/posts/?slug=${slug}`).then(res => res.data);
  const postFullData = postsFullData[0];

  
  const commentsFullData = await WordpressRestAPI.get(`/comments?post=${postFullData.id}`).then(res => res.data);
  const relatedPostsFullData = await WordpressRestAPI
    .get(`/posts/?filter[meta_key]=category&filter[meta_value]=${postFullData.acf.category}&exclude=${postFullData.id}`)
    .then(res => res.data);
