...
class Paginator extends Component {
  ...
  pages = (total, items) => parseInt(total / items, 10) + (total % items > 0 ? 1 : 0);
  ...
  handleTextChange({ target: { value } }) {
    ...
    if (total && !isNaN(valueInt) && parseInt(total / items, 10) + 1 >= valueInt) {
        ...
        <Grid container spacing={0}>
          ...
          <Grid item xs={3}>
            <button
              ...
              className={classes.paginateButton}
              style={{
                color: this.pages(total, items) <= currentPage ? '#ccc' : '#3577D4',
