...
import {getCategory, updateCategory, uploadCategoryImage, deleteCategoryImage} from "../../actions";
    ...
    realhandleSubmit = data => {
      ...
      selectImageFile = event =>{
        ...
        reader.onload = e => {
          ...
          this.props.uploadCategoryImage({categoryId: this.props.category.id, file:  e.target.result,
             fileName: files[0].name, categoryName: this.props.category.name}).then(res=> {
      ...
      return (
        ...
        <Form onSubmit={this.props.handleSubmit(this.realhandleSubmit.bind(this))}>
          ...
          <Grid item xs={12} alignItems="flex-start" style={{ paddingTop: 40, textAlign: "left" }}>
            ...
            <div style={{width: 40, right: 0, position: "relative", marginTop: -40}}>
          
            <input type="file" name="fileToUpload"  id="fileToUpload" onChange={this.selectImageFile.bind(this)} style={{display: "none"}}></input>
            ...
            {this.props.category.images&& (this.props.category.images.length === 0 ? 
            ...
            :
             <CarouselTool>
               {this.props.category.images.map(image =>(
                 <div align="right" style={{width: "100%", height: "auto", height: 245, position: "relative", textAlign: "right",
                  backgroundImage: "url("+image.url+")", backgroundSize: "cover"}}>
...
const mapStateToProps = state => {
  ...
  export default connect(
    mapStateToProps,
    { initialize, getCategory, updateCategory, uploadCategoryImage, deleteCategoryImage}
  )(
