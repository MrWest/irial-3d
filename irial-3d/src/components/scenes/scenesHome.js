...
import { fetchScenes, sortScenes } from "../../actions";
...
class ScenesHome extends Component {
  ...
  componentWillMount() {
    if(!isServer) {
      ...
      this.props.sortScenes(category).then(() => {
  ...
  handleChange = event => {
    if(this.state.sort !==  event.target.value)
    {
      ...
      this.props.history.push("/scenes/"+event.target.value);
    }
...
export default connect(mapStateTopProps, {fetchScenes, sortScenes })(withStyles(styles)(ScenesHome));
