...
import {
  selectTexture, addTextureComment, updateTextureComment,
  deleteTextureComment, rateTexture, getCategory, addToCart
} from '../../actions';
...
const RelatedTextureCard = ({ classes, texture }) => (
  ...
  <div style={{ padding: 4, border: '1px solid #dddddd', borderRadius: 4, cursor: 'pointer' }}>
      ...
      <p style={{ textAlign: 'center', marginTop: 4, color: '#4d4e53', fontFamily: 'Roboto' }}>{texture.name}</p>
  </div>
  </Link>
);
...
class TextureDetails extends Component {
 ...
 handleOnRateClick = (value) => {
   ...
   rateTexture({ id_texture: texture.id, id_user: sign.loginInfo.id, rate: value });
 }
 
 handleAddItem = openCart => {
  const { texture, category, section, addToCart } = this.props;
  addToCart({ id_item: texture.id, name: texture.name, image: texture.images[0].url, price: texture.price,
     lumion_version: texture.lumion_version, section, category, destination: texture.ownerInfo.stripe_account_id, file: texture.server_path, type: 'texture' }, openCart);
 }

 render() {
   const { classes, texture, category, section, language, addTextureComment, updateTextureComment, deleteTextureComment } = this.props;
       ...
       <Grid container justify="center">
         <Grid container spacing={3} className={classes.center}>
           ...
           <Grid item md={8} xs={12}>
             <ImageGallery items={texture.images.map((image) => ({
               original: image.url,
               thumbnail: image.url,
             }))}
             />
              <div style={{paddingTop: 20}}>
                <CommentsTool service="texture" comments={texture.comments} idService={texture.id} serviceUrl={"/texture/"}  onRate={this.handleOnRateClick}
                rate={texture.rate} addComment={addTextureComment} updateComment={updateTextureComment} deleteComment={deleteTextureComment}></CommentsTool>
               </div>
           </Grid>
           <Grid item md={4} xs={12}>
             <p className={classes.textureName}>{texture.name}</p>
             <p className={classes.textureText}>{`${section.name}/${category.name}`}</p>
             <p className={classes.textureText}>{texture.full_description}</p>
             <p className={classes.textureText}><strong>{language.LumionVersion}: </strong>{texture.lumion_version}</p>
             <p className={classes.texturePrice}><strong>{language.Price}: </strong>{`${texture.price} ${texture.currency}`}</p>
             <Grid container spacing={2} style={{ paddingTop: 24 }}>
               ...
               (
                 <Grid xs={12}>
                 <p className={classes.textureText} style={{ marginTop: 24 }}><strong>{language.CreatorInfo}: </strong></p>
                 <Grid container spacing={2} itemsAlign="center">
                   <Grid item>
                      <img style={{height: 38, width: 38, borderRadius: 19}}
                      src={texture.ownerInfo.picture || "./static/images/public/user.png"} />
                    </Grid>
                    <Grid item> 
                      <p className={classes.textureText} style={{ marginBottom: 8 }}>{texture.ownerInfo.first_name}</p>
                      <p className={classes.textureText} style={{ marginBottom: 0 }}>{texture.ownerInfo.last_name}</p>
                    </Grid>
                     <Grid item>
                      <p className={classes.textureText} style={{ marginBottom: 8 }}><strong>{section.name}</strong></p>
                      <p className={classes.textureText} style={{ marginBottom: 0, textAlign: 'center' }}>{texture.ownerInfo.owner_textures}</p>
                    </Grid>
                  </Grid>
               </Grid>
               )
               ...
               {texture.tags && (
                 <Grid container>
                   <p className={classes.textureText} style={{ marginBottom: 16, marginTop: 24 }}><strong>{language.Tags}:</strong></p>
                    ...
                    <Grid container spacing={2}>
                      {texture.relatedTextures.map(rtexture => (
                        <Grid key={rtexture.id} item xs={6}>
                          <RelatedTextureCard classes={classes} texture={rtexture} />
                         </Grid>
                      ))}
                    </Grid>
                </Grid>
