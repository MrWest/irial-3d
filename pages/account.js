    ...
    return (
      <AccountHome
        ...
        stripe_account_status={
          profileStipeInfo ? profileStipeInfo.stripe_account_status : "error"
        }
      />
    );
  }
...
const handleAccount = async (stripe_user_id, state, reduxStore) => {
  ...
  if (account.id) {
    const profileStipeInfo = await updateStripeAccountInfoServer({
      id: state,
      stripe_account_id: account.id,
      stripe_account_type: account.type,
      stripe_account_status:
        requirements && requirements.disabled_reason ? "error" : "verified",
    });
    console.log("profileStipeInfo", profileStipeInfo);

    if (profileStipeInfo.stripe_account_id) {
      var stripeAccountIdsummary = profileStipeInfo.stripe_account_id.replace(
        "acct_",
        ""
      );
      console.log(
        "stripeAccountIdsummary",
        reverseString(stripeAccountIdsummary)
      );
      const linkInfo = await getStripeAccountLoginLink({
        id: profileStipeInfo.stripe_account_id,
        pid: profileStipeInfo.id,
        scaid: reverseString(stripeAccountIdsummary),
      });
      console.log(linkInfo);
      return {
        profileStipeInfo: { ...profileStipeInfo, linkUrl: linkInfo.url },
      };
    }
...
Account.getInitialProps = async function ({
  ...
  if (code && state) {
    ...
    return stripe_user_id
      ? handleAccount(stripe_user_id, state, reduxStore)
      : {};
  }
