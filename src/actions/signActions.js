...
export const logIn = (logIn) => async (dispatch) => {
  ...
  if (logIn.picture) {
    if (logInRslt.type) logIn.type = logInRslt.type;
...
export const signUp = (signUp) => async (dispatch) => {
  ...
  if (signUp.picture) uploadInfo.append("picture", signUp.picture);

  const userDb = await DashBoard.post("/users/add_user.php", uploadInfo, {
    headers,
  });
...
export const getPasswordAccess = (idreset) => async (dispatch) => {
  ...
  const promises = await DashBoard.get(
    "/users/get_password_access.php" + uploadInfo
  );
