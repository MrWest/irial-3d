...
export const sortTextures = (
  ...
  const queryAPI = await DashBoard.post(
    "/textures/get_textures_count.php" + generatePHPParameters({ lang })
  );
...
export const sortTexturesByCategoryUser = (info) => async (dispatch) => {
  ...
  //  else
  await DashBoard.get(
    "/textures/get_textures_category_user.php" + completeUrl
  ).then(async (texturesDb) => {
    var texturesRslt = texturesDb.data.slice();
...
export const sortTexturesUser = (user) => async (dispatch) => {
  ...
  texturesDb = await DashBoard.post(
    "/textures/get_textures_user.php" + generatePHPParameters({ user, lang })
  );
...
export const updateTexture = (texture) => async (dispatch) => {
  ...
  if (texture.status) {
    ...
    await DashBoard.post("/textures/update_texture_status.php", statusInfo, {
      headers,
    });
  } else {
  ...
  const texturesImagesDb = await DashBoard.get(
    "/textures/get_texture_images.php" +
      generatePHPParameters({ idTexture: textureRslt.id })
  );
...
export const selectTexture = (id) => async (dispatch) => {
  ...
  const textureOwnerInfo = await DashBoard.get(
    "/textures/get_texture_owner_info.php" +
      generatePHPParameters({ user: texturesRslt.id_user })
  );
  texturesRslt = { ...texturesRslt, ownerInfo: { ...textureOwnerInfo.data } };

  const textureTags = await DashBoard.get(
    "/tags/get_texture_tags.php" + generatePHPParameters({ id, lang })
  );
  texturesRslt = { ...texturesRslt, tags: textureTags.data };

  const textureTagsReleted = await DashBoard.get(
    "/tags/get_texture_tags_related_textures.php" +
...
export const deleteTexture = (id) => async (dispatch) => {
  const categoriesDb = await DashBoard.post(
    "/textures/delete_texture.php" + generatePHPParameters({ id })
  );
...
export const uploadTextureImage = (info) => async (dispatch) => {
  ...
  if (info.file !== null) {
    ...
    const textureAPI = await DashBoard.post(
      "/textures/upload_texture_image.php",
      uploadInfo,
...
export const uploadTextureFile = (info) => async (dispatch) => {
  ...
  if (info.file !== null) {
    ...
    const textureAPI = await DashBoard.post(
      "/textures/upload_texture_file.php",
      uploadInfo,
      { headers, onUploadProgress: info.onProgress }
    );
...
export const deleteTextureImage = (id) => async (dispatch) => {
  const categoriesDb = await DashBoard.post(
    "/textures/delete_texture_image.php" + generatePHPParameters({ id })
  );
};
...
export const uploadTextureVideo = (info) => async (dispatch) => {
  ...
  if (info.file !== null) {
    ...
    const textureAPI = await DashBoard.post(
      "/textures/upload_texture_video.php",
      uploadInfo,
      { headers, onUploadProgress: info.onProgress }
    );
...
export const deleteTextureVideo = (id) => async (dispatch) => {
  const categoriesDb = await DashBoard.post(
    "/textures/delete_texture_video.php" + generatePHPParameters({ id })
  );
};
...
export const updateTextureComment = (comment) => async (dispatch) => {
  ...
  const commentDb = await DashBoard.post(
    "/textures/update_texture_comment.php",
    uploadInfo,
    { headers }
  );
...
export const addTextureComment = (comment) => async (dispatch) => {
  ...
  const commentDb = await DashBoard.post(
    "/textures/add_texture_comment.php",
    uploadInfo,
    { headers }
  );
...
export const deleteTextureComment = (id) => async (dispatch) => {
  ...
  await DashBoard.post("/textures/delete_texture_comment.php", uploadInfo, {
    headers,
  });
...
export const rateTexture = (info) => async (dispatch) => {
  ...
  const lang = getLanguage();
  const texturesDb = await DashBoard.post(
    "/textures/get_texture.php" +
      generatePHPParameters({ id: info.id_texture, lang })
  );
  ...
  const textureTags = await DashBoard.get(
    "/tags/get_texture_tags.php" +
      generatePHPParameters({ id: texturesRslt.id, lang })
  );
  texturesRslt = { ...texturesRslt, tags: textureTags.data };

  const textureTagsReleted = await DashBoard.get(
    "/tags/get_texture_tags_related_textures.php" +
      generatePHPParameters({ id: texturesRslt.id })
  );
  texturesRslt = { ...texturesRslt, relatedTextures: textureTagsReleted.data };
