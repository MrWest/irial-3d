...
export const sortProjects = (
  ...
  await DashBoard.get("/projects/get_projects.php" + completeUrl).then(
    async (projectsDb) => {
      ...
      const promises = projectsRslt.map(async (project) => {
        ...
        const projectOwnerInfo = await DashBoard.get(
          "/projects/get_project_owner_info.php" +
            generatePHPParameters({ user: project.id_user })
        );
  ...
  const queryAPI = await DashBoard.post(
    "/projects/get_projects_count.php" + generatePHPParameters({ lang })
  );
...
export const sortProjectsByCategoryUser = (info) => async (dispatch) => {
  ...
  //  else
  await DashBoard.get(
    "/projects/get_projects_category_user.php" + completeUrl
  ).then(async (projectsDb) => {
    var projectsRslt = projectsDb.data.slice();
...
export const sortProjectsUser = (user) => async (dispatch) => {
  ...
  projectsDb = await DashBoard.post(
    "/projects/get_projects_user.php" + generatePHPParameters({ user, lang })
  );
...
export const updateProject = (project) => async (dispatch) => {
  ...
  if (project.status) {
    ...
    await DashBoard.post("/projects/update_project_status.php", statusInfo, {
      headers,
    });
  } else {
  ...
  const projectsImagesDb = await DashBoard.get(
    "/projects/get_project_images.php" +
      generatePHPParameters({ idProject: projectRslt.id })
  );
...
export const selectProject = (id) => async (dispatch) => {
  ...
  const projectOwnerInfo = await DashBoard.get(
    "/projects/get_project_owner_info.php" +
      generatePHPParameters({ user: projectsRslt.id_user })
  );
  projectsRslt = { ...projectsRslt, ownerInfo: { ...projectOwnerInfo.data } };

  const projectTags = await DashBoard.get(
    "/tags/get_project_tags.php" + generatePHPParameters({ id, lang })
  );
  projectsRslt = { ...projectsRslt, tags: projectTags.data };

  const projectTagsReleted = await DashBoard.get(
    "/tags/get_project_tags_related_projects.php" +
      generatePHPParameters({ id })
  );
...
export const deleteProject = (id) => async (dispatch) => {
  const categoriesDb = await DashBoard.post(
    "/projects/delete_project.php" + generatePHPParameters({ id })
  );
...
export const uploadProjectImage = (info) => async (dispatch) => {
  ...
  if (info.file !== null) {
    ...
    const projectAPI = await DashBoard.post(
      "/projects/upload_project_image.php",
      uploadInfo,
...
export const uploadProjectFile = (info) => async (dispatch) => {
  ...
  if (info.file !== null) {
    ...
    const projectAPI = await DashBoard.post(
      "/projects/upload_project_file.php",
      uploadInfo,
      { headers, onUploadProgress: info.onProgress }
    );
...
export const deleteProjectImage = (id) => async (dispatch) => {
  const categoriesDb = await DashBoard.post(
    "/projects/delete_project_image.php" + generatePHPParameters({ id })
  );
};
...
export const uploadProjectVideo = (info) => async (dispatch) => {
  ...
  if (info.file !== null) {
    ...
    const projectAPI = await DashBoard.post(
      "/projects/upload_project_video.php",
      uploadInfo,
      { headers, onUploadProgress: info.onProgress }
    );
...
export const deleteProjectVideo = (id) => async (dispatch) => {
  const categoriesDb = await DashBoard.post(
    "/projects/delete_project_video.php" + generatePHPParameters({ id })
  );
};
...
export const updateProjectComment = (comment) => async (dispatch) => {
  ...
  const commentDb = await DashBoard.post(
    "/projects/update_project_comment.php",
    uploadInfo,
    { headers }
  );
...
export const addProjectComment = (comment) => async (dispatch) => {
  ...
  const commentDb = await DashBoard.post(
    "/projects/add_project_comment.php",
    uploadInfo,
    { headers }
  );
...
export const deleteProjectComment = (id) => async (dispatch) => {
  ...
  await DashBoard.post("/projects/delete_project_comment.php", uploadInfo, {
    headers,
  });
...
export const rateProject = (info) => async (dispatch) => {
  ...
  const lang = getLanguage();
  const projectsDb = await DashBoard.post(
    "/projects/get_project.php" +
      generatePHPParameters({ id: info.id_project, lang })
  );
  ...
  const projectTags = await DashBoard.get(
    "/tags/get_project_tags.php" +
      generatePHPParameters({ id: projectsRslt.id, lang })
  );
  projectsRslt = { ...projectsRslt, tags: projectTags.data };

  const projectTagsReleted = await DashBoard.get(
    "/tags/get_project_tags_related_projects.php" +
      generatePHPParameters({ id: projectsRslt.id })
  );
  projectsRslt = { ...projectsRslt, relatedProjects: projectTagsReleted.data };
