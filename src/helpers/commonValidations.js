...
const phoneValidation = (country) =>
  country === "US" ? [] : [required({ msg: "Required" })];
  ...
  ["US", "USA"].includes(country)
    ? [
        ...
        format({
          with: /^\d{5}(-\d{4})?$/i,
          message: {
            defaultMessage: "Must be 5 or 9 digits. Ex: 54321, 98765-4321",
          },
        }),
      ]
    : [required({ msg: "Required" })];
...
const numberIdValidation = (country) =>
  ["US", "USA"].includes(country)
    ? [required({ msg: "Required" }), length({ max: 4 })]
    : [required({ msg: "Required" })];
...
const runFieldValidations = (fieldsToValidate, values, validations) => {
  ...
  if (fieldsToValidate) {
    fieldsToValidate.forEach((field) => {
      ...
      const error = fieldValidations
        .map((fv) => fv(fieldValue, values))
        .find((x) => x);
...
const fieldValidation = (field, values) => {
  switch (field) {
    ...
    case "naccount":
      return [
        required({ msg: "Please, provide a valid name for the account" }),
      ];
    ...
    case "rnumber":
      return [
        required({ msg: "Please, provide a valid account routing number" }),
      ];
