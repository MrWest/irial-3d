...
(moment(monday).year() === moment(sunday).year() ? 
 moment(monday).format('YYYY') : `${ moment(monday).format('YYYY')} / ${ moment(sunday).format('YYYY')}`);

const getMonth = ( monday, sunday ) => 
(moment(monday).month() === moment(sunday).month() ? 
 moment(monday).format('MMMM') : `${ moment(monday).format('MMMM')} / ${ moment(sunday).format('MMMM')}`);
...
const DayOfWeek = ({ dayOfWeek, dayOfMonth, isSeledted, onClick }) => (
    <Grid container justify="center">
        <Grid item>
            <button onClick={onClick} style={{ border: 0, background: 'transparent', padding: 0, outline: 'none', cursor: 'pointer'}}>
            <p style={{ marginBottom: 16, textAlign: 'center' }}>{dayOfWeek}</p>
            <Grid container justify="center" alignItems="center" >
                <Grid item  container justify="center" alignItems="center"  style={{ height: 56, width: 56, borderRadius: 28, background: isSeledted ? '#efdfdf' : '#dfdfdf', border: '1px solid #fdfdfd' }}>
                   {dayOfMonth}
                </Grid>
            </Grid>
            </button>
...
const WeekOfMonth = ({ week, selectedDate, setSelectedDate}) => (
    <div>
        <Grid container justify="center">
                    <Grid item>
                    <h2>{week.month} <span style={{ opacity: 0.3, fontSize: 12}}>{week.year} </span></h2> 
                    </Grid>
                </Grid>
                <Grid container >
                    {week.weekDays.map(wd => (
                        <Grid key={wd.date()} item xs>
                            <DayOfWeek dayOfWeek={moment(wd).format('ddd')} dayOfMonth={moment(wd).format('DD')} 
                            isSeledted={moment(wd).format('YYYY-MM-DD') === moment(selectedDate).format('YYYY-MM-DD')} onClick={() => setSelectedDate(wd)} />
                        </Grid>
                    ))}
                </Grid>
    </div>
...
const HorizontalCalendar = props => {
    ...
    const [ previousWeek, setPreviousWeek ] = useState(generateWeek(moment(currentDate).add(-7, 'days')));
    const [ nextWeek, setNextWeek ] = useState(generateWeek(moment(currentDate).add(7, 'days')));
    ...
    useEffect(() => {
        if(!contentWidth) {
            ...
            setDaysAreaWidth(entireComponent.clientWidth - (buttonArea.clientWidth *2));
        }
            ...
            <Grid item xs style={{ width: daysAreaWidth, display: 'grid', overflow: 'hidden', flexBasis: 'unset' }}>
                <Grid container style={{ flexWrap: 'nowrap', transition: translationIndex !== 1 ? 'transform 0.8s ease-out 0.2s' : 'none', transform: `translate3d(-${translationIndex*33.33}%, 0px, 0px)` }}>
                    <Grid item  style={{ width: daysAreaWidth}}>
                        <WeekOfMonth week={previousWeek} selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
                    </Grid>
                    <Grid item  style={{ width: daysAreaWidth}}>
                        <WeekOfMonth week={currentWeek} selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
                    </Grid>
                    <Grid item  style={{ width: daysAreaWidth}}>
                        <WeekOfMonth week={nextWeek} selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
                    </Grid>
                </Grid>
            </Grid>
            <Grid item>
