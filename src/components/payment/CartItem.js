  ...
  <div className={classes.cartItemContainer}>
    <Grid container alignItems="center" spacing={2}>
      ...
      <Grid item xs>
        ...
        >{`${item.section.name}/${item.category.name}`}</p>
        <p className={classes.cartItemName}>
          {item.lumion_version}{" "}
          <span className={classes.cartSmaltext}>Lumion</span>
        </p>
      </Grid>
      <Grid item>
      ...
      <Grid item>
        <StylessButton onClick={() => removeFromCart(item.id)}>
...
export default connect(null, { removeFromCart, emptyCart })(
  withStyles(styles)(withRouter(CartItem))
);
