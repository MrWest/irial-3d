...
const RelatedPosts = ({ classes, relatedPosts, category }) => {
  ...
  .length > 0? (  <section className={classes.RelatedPosts}>
      ...
      <main className={classes.center}>
        <div className={classes.sectionContainer}>
          ...
          <Grid container spacing={6}>
            {relatedPosts && managePosts(relatedPosts).map(rp => (
              <Grid key={rp.id} item xs={12} md={4} className={classes.CardContainer}>
