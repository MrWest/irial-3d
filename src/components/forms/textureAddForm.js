...
import { addTexture, notifyActivity } from "../../actions";
...
class TextureAddForm extends React.Component {
  ...
  realhandleSubmit = (data) => {
    ...
    addTexture(data).then((texture) => {
      if (texture.id) {
        let message =
          sign.loginInfo.first_name +
          " " +
          sign.loginInfo.last_name +
          " has added a new texture business to vinalestraveler called: " +
          data.name;

        let subject = "New texture";

        let email = sign.loginInfo.email;

        notifyActivity({ message, subject, email });

        history.push(`/textureedit/${texture.id}`);
      }
    });
  };
                ...
                <Grid item xs={12}>
                  <Grid container>
                    ...
                    <Grid item alignItems="baseline">
                      ...
                      >
                        {language.AddFormTittle.format(language.Texture)}
                      </Typography>
                      <Grid container>
                        <Grid item xs={2}>
                      ...
                      >
                        {language.AddFormText.format(language.Texture)}
                      </Typography>
                    </Grid>
                  </Grid>
                </Grid>
...
export default connect(mapStateToProps, { initialize, addTexture })(
  reduxForm({ form: "textureAddForm", validate })(
    withStyles(styles)(withRouter(TextureAddForm))
  )
);
