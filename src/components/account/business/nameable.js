...
class Nameable extends Component {
  ...
  render() {
    ...
    if(size === "large")
     innerPadding = "50px 10px"
     if(size === "medium")
     innerPadding = "30px 10px"
    

    return (
        <a id={id} onClick={onClick}  style={{cursor: "pointer"}}>
        <main style={{marginBottom: 5,padding: innerPadding}} className={selected? classes.selected: classes.normal}>
        
        <Grid container>
            <Grid item xs={2}>
                <div style={{border: "1px #3577d4 solid", background: "#fefefe", height: 50, width: 50, borderRadius: 25, display: "table"}}>
